...
impl Engine{
 ...
 fn candidate_rows_with_lookups(&self,p:&[Predicate])->Option<RowPlan>{
  let q=match self.query_map(p){Some(q)=>q,None=>return Some(RowPlan{rows:Vec::new(),lookups:0,fully_covered:true})};
  #[derive(Clone)]struct Candidate{count:usize,index:usize,key:u64,columns:Vec<usize>}
  let mut applicable=Vec::new();for(i,h)in self.row_hier.iter().enumerate(){if h.columns.iter().all(|c|q.contains_key(c)){let key=self.hierarchy_key(&h.columns,&q)?;let count=h.data.row_count(key);if count==0{return Some(RowPlan{rows:Vec::new(),lookups:1,fully_covered:true})}applicable.push(Candidate{count,index:i,key,columns:h.columns.clone()});}}
  if applicable.is_empty(){return None}let considered=applicable.len()as u64;let mut uncovered:HashSet<usize>=q.keys().copied().collect();let mut selected=Vec::new();let mut used=vec![false;applicable.len()];
  while !uncovered.is_empty(){let mut best:Option<(usize,usize,usize)>=None;for(ci,c)in applicable.iter().enumerate(){if used[ci]{continue}let gain=c.columns.iter().filter(|x|uncovered.contains(x)).count();if gain==0{continue}match best{None=>best=Some((ci,gain,c.count)),Some((_,bg,bc))=>{let left=c.count as u128*bg as u128;let right=bc as u128*gain as u128;if left<right||(left==right&&c.count<bc){best=Some((ci,gain,c.count));}}}}let Some((ci,_,_))=best else{break};used[ci]=true;for col in &applicable[ci].columns{uncovered.remove(col);}selected.push(applicable[ci].clone());}
  if selected.is_empty(){return None}selected.sort_unstable_by_key(|x|x.count);let first=&selected[0];let mut rows=self.row_hier[first.index].data.rows(first.key);for c in &selected[1..]{rows=self.row_hier[c.index].data.intersect_rows(c.key,&rows);if rows.is_empty(){break}}
  Some(RowPlan{rows,lookups:considered,fully_covered:uncovered.is_empty()})
 }
 fn candidate_pages_with_lookups(&self,p:&[Predicate])->(Vec<u32>,u64){
  let q=match self.query_map(p){Some(q)=>q,None=>return(Vec::new(),0)};let mut a=Vec::new();for(i,h)in self.page_hier.iter().enumerate(){if h.columns.iter().all(|c|q.contains_key(c)){let Some(k)=self.hierarchy_key(&h.columns,&q)else{return(Vec::new(),0)};let n=h.data.page_count(k);if n==0{return(Vec::new(),1)}a.push((n,i,k));}}
  let lookups=a.len()as u64;if a.is_empty(){return((0..self.pages).collect(),0)}a.sort_unstable_by_key(|x|x.0);let(_,i,k)=a[0];let mut out=self.page_hier[i].data.pages(k);for&(_,i,k)in &a[1..]{out=self.page_hier[i].data.intersect_pages(k,&out);if out.is_empty(){break}}(out,lookups)
 }
