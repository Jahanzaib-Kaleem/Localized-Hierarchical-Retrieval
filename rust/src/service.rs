...
impl Default for ServiceConfig {
    fn default() -> Self {
        Self {
            ...
            max_query_limit: default_query_limit(),
            max_rows_examined: default_rows_examined(),
            max_query_timeout_ms: default_timeout_ms(),
            max_mutation_ops: default_mutation_ops(),
            max_batch_rows: default_batch_rows(),
            max_sort_records: default_sort_records(),
            max_dictionary_run_bytes: default_dictionary_bytes(),
    ...
    pub fn validate(&self) -> io::Result<()> {
        ...
        if !bind.ip().is_loopback() && !self.behind_tls_proxy {
            return Err(io::Error::new(
                ...
                "refusing non-loopback cleartext bind; set behind_tls_proxy=true only when TLS/private transport is enforced upstream",
            ));
        }
        if !bind.ip().is_loopback() && self.api_keys.is_empty() {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "remote service listeners require at least one API key",
            ));
        }
        if self.max_body_bytes == 0
            ...
            || self.max_query_timeout_ms == 0
            || self.max_mutation_ops == 0
            || self.max_batch_rows == 0
            || self.max_sort_records == 0
            || self.max_dictionary_run_bytes == 0
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "service resource ceilings must all be greater than zero",
            ));
        }
        ...
        for key in &self.api_keys {
            if key.id.trim().is_empty() || key.token.len() < 16 {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "API key IDs must be non-empty and tokens must contain at least 16 characters",
                ));
            }
            if !ids.insert(key.id.clone()) || !tokens.insert(hash_token(&key.token)) {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "duplicate API key id or token",
                ));
            }
        }
        Ok(())
...
impl ApiError {
    fn new(status: StatusCode, request_id: u64, message: impl Into<String>) -> Self {
        Self { status, request_id, message: message.into() }
    }
}
...
async fn begin_request(
    ...
    if state.config.rate_limit_per_minute > 0 {
        let mut rates = state.rates.lock().map_err(|_| {
            ApiError::new(StatusCode::INTERNAL_SERVER_ERROR, request_id, "rate limiter lock poisoned")
        })?;
        let now = Instant::now();
...
fn append_audit(state: &ServiceState, event: &AuditEvent<'_>) -> io::Result<()> {
    let path = state
        ...
        .audit_log
        .clone()
        .unwrap_or_else(|| state.root.join("audit").join("audit.jsonl"));
...
async fn readyz(State(state): State<ServiceState>) -> Response {
    ...
    match tokio::task::spawn_blocking(move || VersionedDataset::open(root).map(|_| ())).await {
        ...
        Ok(Err(error)) => (StatusCode::SERVICE_UNAVAILABLE, Json(json!({"status":"not_ready","error":error.to_string()}))).into_response(),
...
async fn query(
    ...
    .await
    .map_err(|e| join_error(guard.request_id, e))?;
    match result {
        Ok(response) => {
        ...
        Err(error) => {
            state.metrics.query_failures.fetch_add(1, Ordering::Relaxed);
            state.metrics.errors.fetch_add(1, Ordering::Relaxed);
...
fn mutation_config(options: &WriteOptions, config: &ServiceConfig) -> io::Result<MutationConfig> {
    ...
    if batch_rows == 0 || batch_rows > config.max_batch_rows
        || max_sort_records == 0 || max_sort_records > config.max_sort_records
        || dictionary_run_bytes == 0 || dictionary_run_bytes > config.max_dictionary_run_bytes
    {
...
async fn mutate(
    ...
    let config = mutation_config(&request.options, &state.config)
        .map_err(|e| ApiError::new(io_status(&e), guard.request_id, e.to_string()))?;
    let root = state.root.clone();
    let operations = request.mutations;
    let count = operations.len();
    let result = tokio::task::spawn_blocking(move || apply_mutations_delta(root, &operations, &config))
        .await
        .map_err(|e| join_error(guard.request_id, e))?;
    match result {
        Ok(report) => {
            ...
            let _ = append_audit(&state, &AuditEvent {
                ...
                action: "mutate", success: true, detail: json!({"operations":count,"report":report}),
            });
        ...
        Err(error) => {
            ...
            let _ = append_audit(&state, &AuditEvent {
                timestamp_ms: now_ms(), request_id: guard.request_id, actor: &guard.actor,
                action: "mutate", success: false, detail: json!({"operations":count,"error":error.to_string()}),
            });
            Err(ApiError::new(io_status(&error), guard.request_id, error.to_string()))
        }
    }
...
async fn compact(
    ...
    let config = compaction_config(&request.options, &state.config)
        .map_err(|e| ApiError::new(io_status(&e), guard.request_id, e.to_string()))?;
    let root = state.root.clone();
    let result = tokio::task::spawn_blocking(move || compact_dataset(root, &config))
        .await
        .map_err(|e| join_error(guard.request_id, e))?;
    match result {
        Ok(report) => {
            state.metrics.compactions.fetch_add(1, Ordering::Relaxed);
            state.metrics.admin_actions.fetch_add(1, Ordering::Relaxed);
            let _ = append_audit(&state, &AuditEvent {
                timestamp_ms: now_ms(), request_id: guard.request_id, actor: &guard.actor,
                action: "compact", success: true, detail: json!({"report":report}),
            });
            Ok(Json(json!({"request_id":guard.request_id,"result":report})))
        }
        Err(error) => {
            ...
            let _ = append_audit(&state, &AuditEvent {
                timestamp_ms: now_ms(), request_id: guard.request_id, actor: &guard.actor,
                action: "compact", success: false, detail: json!({"error":error.to_string()}),
            });
            Err(ApiError::new(io_status(&error), guard.request_id, error.to_string()))
        }
    }
...
async fn stats(State(state): State<ServiceState>, headers: HeaderMap) -> Result<Json<Value>, ApiError> {
    let guard = begin_request(&state, &headers, ServiceRole::Read).await?;
    let root = state.root.clone();
    let result = tokio::task::spawn_blocking(move || dataset_stats(root)).await.map_err(|e| join_error(guard.request_id, e))?;
    result.map(|value| Json(json!({"request_id":guard.request_id,"result":value})))
        .map_err(|e| ApiError::new(io_status(&e), guard.request_id, e.to_string()))
}
...
async fn workload(State(state): State<ServiceState>, headers: HeaderMap) -> Result<Json<Value>, ApiError> {
    let guard = begin_request(&state, &headers, ServiceRole::Read).await?;
    let root = state.root.clone();
    let result = tokio::task::spawn_blocking(move || workload_report(root)).await.map_err(|e| join_error(guard.request_id, e))?;
    result.map(|value| Json(json!({"request_id":guard.request_id,"result":value})))
        .map_err(|e| ApiError::new(io_status(&e), guard.request_id, e.to_string()))
}
...
async fn generations(State(state): State<ServiceState>, headers: HeaderMap) -> Result<Json<Value>, ApiError> {
    let guard = begin_request(&state, &headers, ServiceRole::Read).await?;
    let root = state.root.clone();
    let result = tokio::task::spawn_blocking(move || list_generations(root)).await.map_err(|e| join_error(guard.request_id, e))?;
    result.map(|value| Json(json!({"request_id":guard.request_id,"result":value})))
        .map_err(|e| ApiError::new(io_status(&e), guard.request_id, e.to_string()))
}
...
async fn vacuum(
    State(state): State<ServiceState>, headers: HeaderMap, Json(request): Json<VacuumRequest>,
) -> Result<Json<Value>, ApiError> {
    let guard = begin_request(&state, &headers, ServiceRole::Admin).await?;
    let root = state.root.clone();
    let result = tokio::task::spawn_blocking(move || vacuum_with_reader_leases(root, request.retain, &request.protect))
...
async fn index_add(State(state): State<ServiceState>, headers: HeaderMap, Json(request): Json<IndexRequest>) -> Result<Json<Value>, ApiError> {
    index_change(state, headers, request, "add").await
}
async fn index_drop(State(state): State<ServiceState>, headers: HeaderMap, Json(request): Json<IndexRequest>) -> Result<Json<Value>, ApiError> {
    index_change(state, headers, request, "drop").await
}
async fn index_rebuild(State(state): State<ServiceState>, headers: HeaderMap, Json(request): Json<IndexRequest>) -> Result<Json<Value>, ApiError> {
    index_change(state, headers, request, "rebuild").await
}
...
async fn index_change(state: ServiceState, headers: HeaderMap, request: IndexRequest, action: &'static str) -> Result<Json<Value>, ApiError> {
    ...
    if request.columns.len() < 2 {
        return Err(ApiError::new(StatusCode::BAD_REQUEST, guard.request_id, "accelerator indexes require at least two columns"));
    }
    let max_sort = request.max_sort_records.unwrap_or(250_000);
    if max_sort == 0 || max_sort > state.config.max_sort_records {
        return Err(ApiError::new(StatusCode::BAD_REQUEST, guard.request_id, "max_sort_records exceeds service ceiling"));
    }
    let root = state.root.clone();
    let columns = request.columns.clone();
    let result = tokio::task::spawn_blocking(move || match action {
        "add" => add_index(root, &columns, max_sort),
        "drop" => drop_index(root, &columns),
        _ => rebuild_index(root, &columns, max_sort),
    }).await.map_err(|e| join_error(guard.request_id, e))?;
    match result {
        Ok(report) => {
            state.metrics.admin_actions.fetch_add(1, Ordering::Relaxed);
    ...
    if let Ok(status) = fs::read_to_string("/proc/self/status") {
        for line in status.lines() {
            if let Some(value) = line.strip_prefix("VmRSS:") {
                if let Some(kib) = value.split_whitespace().next().and_then(|x| x.parse::<u64>().ok()) {
                    out.push(("lhr_process_rss_bytes", kib.saturating_mul(1024)));
                }
            }
        }
    }
...
async fn metrics(State(state): State<ServiceState>, headers: HeaderMap) -> Result<Response, ApiError> {
    ...
    response.headers_mut().insert(axum::http::header::CONTENT_TYPE, "text/plain; version=0.0.4".parse().unwrap());
    drop(guard);
    Ok(response)
}
...
pub async fn serve(root: impl AsRef<Path>, config: ServiceConfig) -> io::Result<()> {
    ...
    for key in &config.api_keys {
        principals.insert(hash_token(&key.token), Principal { id: key.id.clone(), role: key.role });
    }
    ...
    // Refuse to start before the current logical dataset can be opened.
    VersionedDataset::open(&state.root)?;
    ...
    let app = Router::new()
        ...
        .route("/v1/admin/vacuum", post(vacuum))
        .route("/v1/admin/recover", post(recover))
        .route("/v1/admin/index/add", post(index_add))
        .route("/v1/admin/index/drop", post(index_drop))
        .route("/v1/admin/index/rebuild", post(index_rebuild))
...
mod tests {
    ...
    fn role_order_matches_authorization_strength() {
        assert!(ServiceRole::Admin > ServiceRole::Write);
        assert!(ServiceRole::Write > ServiceRole::Read);
    }
}
