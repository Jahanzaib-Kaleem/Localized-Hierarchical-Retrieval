...
export function OperationsPage() {
  ...
  const compact = useMutation({ mutationFn: () => api.compact(bucket), onSuccess: finish })
  const vacuum = useMutation({ mutationFn: () => api.vacuum(retain, bucket), onSuccess: finish })
  const recover = useMutation({ mutationFn: () => api.recover(bucket), onSuccess: finish })
  const activeError = compact.error ?? vacuum.error ?? recover.error
  return <div className="page stack stack--lg">
    <PageHeader eyebrow="Administrative control" title="Operations" description="Run bounded maintenance transactions deliberately inside one bucket. No admin operation is retried automatically by Studio." action={buckets.data?.length ? <label className="bucket-select"><span>Bucket</span><select value={bucket} onChange={(event) => setBucket(event.target.value)}>{buckets.data.map((item) => <option key={item.id} value={item.id} disabled={!item.ready}>{item.name}{item.ready ? '' : ' · empty'}</option>)}</select></label> : undefined} />
    {activeError ? <Notice title="Operation failed">{activeError.message}</Notice> : null}
    <div className="operation-grid">
      <Panel title="Compaction" eyebrow="Base + deltas"><div className="operation-card"><p>Stream visible logical rows into a fresh compact generation, rebuild exact structures, then publish atomically.</p><button className="button" disabled={!activeBucket?.ready || compact.isPending} onClick={() => window.confirm('Start compaction?') && compact.mutate()}>{compact.isPending ? 'Compacting…' : 'Compact dataset'}</button></div></Panel>
      <Panel title="Vacuum" eyebrow="Lease-aware retention"><div className="operation-card"><p>Remove old generations that are outside retention and not protected by active reader leases.</p><label className="field"><span>Retain newest</span><input type="number" min={1} max={100} value={retain} onChange={(event) => setRetain(Math.max(1, Number(event.target.value) || 1))} /></label><button className="button" disabled={!activeBucket?.ready || vacuum.isPending} onClick={() => window.confirm(`Vacuum generations while retaining ${retain}?`) && vacuum.mutate()}>{vacuum.isPending ? 'Vacuuming…' : 'Vacuum catalog'}</button></div></Panel>
      <Panel title="Recovery" eyebrow="Verification-based"><div className="operation-card"><p>Repoint CURRENT to the newest fully verified published generation and clean abandoned staging work.</p><button className="button" disabled={!activeBucket?.ready || recover.isPending} onClick={() => window.confirm('Run catalog recovery?') && recover.mutate()}>{recover.isPending ? 'Recovering…' : 'Run recovery'}</button></div></Panel>
    </div>
